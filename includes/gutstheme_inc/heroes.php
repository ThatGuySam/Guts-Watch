...
function heroOrganism($hero) {
	...
	switch ($hero->kind) {
		...
		break;
		case 'welcome'://Default Welcome Hero
			
			ob_start(); 
			
			if( isLive() ){
				$hero->src = 'https://gutschurch.com/wp-content/uploads/2014/10/Watch-Live_Banner-1280x720.jpg';
			}
						...
						<div class="page-header">
							<span class="icon-font hero-header spaced" style="margin: 1em 0;">GUTSchurch</span>
								...
								<div class="hero-ctas hero-ctas-live animated fadeIn animated-3s animated-delay-1s">
									<a href="http://gutschurch.churchonline.org">
		...
		case 'legacy':
			...
			//Default BG //gutschurch.com/wp-content/uploads/2014/10/Legacy-Slate_1080-compressor.png
			$hero->src = '//gutschurch.com/wp-content/uploads/2016/02/0116_GUTS_Legacy_Slide_Web_SC_03.png';
			
			?>
			
			<div class="hero-slide hero-legacy" >
				
				<a href="#heroclose" data-role="none" class="close-frame" style="display: block;"><img src="//gutschurch.com/wp-content/uploads/2016/01/close.png" ></a>
...
<!-- 							<span class="icon-font hero-header spaced" style="margin: 1em 0;">GUTSchurch</span> -->
		...
		case 'invite':
			...
			<div id="hero-invite" class="hero-slide hero-video" data-video="https://player.vimeo.com/video/158845709?byline=0&portrait=0&badge=0&color=d8d8d8&autoplay=1">
				
				<a href="#heroclose" data-role="none" class="close-frame" style="display: block;"><img src="//gutschurch.com/wp-content/uploads/2016/01/close.png" ></a>
				
				<div class="hero-background animated fadeIn animated-3s animated-delay-1s">
					<video autoplay="" muted="" loop="" poster="//gutschurch.s3.amazonaws.com/media/160327_easter2016/Easter-2016-Invite-Hero.gif">
						<source src="//gutschurch.s3.amazonaws.com/media/160327_easter2016/Easter-2016-Invite-Hero.mp4" type="video/mp4">
						<source src="//gutschurch.s3.amazonaws.com/media/160327_easter2016/Easter-2016-Invite-Hero.oggtheora.ogv" type="video/ogg">
						<source src="//gutschurch.s3.amazonaws.com/media/160327_easter2016/Easter-2016-InviteHero.webmhd.webm" type="video/webm">
					</video>
...
<!-- 					<img <?php echo $hero->srcType; ?>="<?php echo $hero->src; ?>" alt="<?php echo $hero->text; ?>"> -->
				...
				<div class="hero-foreground" style="<?php //BG Color Overlay
					...
					<div class="container">
						<div class="page-header">
							<span class="hero-header strong" style="margin: 1em 0; font-weight: 900; font-size: 3em;">Invite someone to Guts Church this Easter.</span>
			...
			<?php $hero->output = ob_get_clean();//End Buffer and assign to variable
			
		break;
		default:
	}
	
	if( !isset($hero->output) ) $hero->output = '';
	
	//Link Wrapper
	if( isset($hero->link) && $hero->link && $hero->link !== "" && !$hero->ctas ){ 
...
class Bars {
	...
	static function handle_shortcode($atts) {
		...
		//Layout Algorithm
		//Focus 12 = (focus + (blur * (count - 1) ) )
		//Rest 12 = (rest * count) + (offset * 2)
		
/*
		if ($count & 1) {//if even
			
			$offset = ( 12 - ( $profiles_count * 2 ) ) / 2;
...
*/
		...
		$focus_class = 'col-md-'.$layout->focus;
		
		$blur_class = 'col-md-'.$layout->blur;
		
		$rest_class = 'col-md-'.$layout->rest;
		
		ob_start(); ?>
			
	        <div class="container-fluid">

				<div class="row youth-programs programs-<?php echo $count; ?>" style="<?php if( $hero_color ) echo "background:".$hero_color.";"; ?>" data-focus-class="<?php echo $focus_class; ?>" data-blur-class="<?php echo $blur_class; ?>" data-rest-class="<?php echo $rest_class; ?>" >
				            ...
				            <?php $p=0; if ( $query->have_posts() ): while ( $query->have_posts() ): $query->the_post();  ?>
					            ...
					            <div class="youth-program bar-bar ease bar-<?php echo $p; ?> <?php echo $rest_class; ?>" rel="" onclick="location.href='<?php the_permalink(); ?>';" style="height: 400px; cursor: pointer; <?php //BG Color Overlay
									if( get_field('page_color') ): 
										?>background: <?php echo get_field('page_color'); ?>; <?php //#000000
									endif; ?>">
		...
		?>
			...
			<script type="text/javascript">
				if ( undefined !== window.jQuery ) { jQuery(function ($) { 'use strict';
  					...
  					var focus_class	= $(".youth-programs").data("focus-class");
					var blur_class	= $(".youth-programs").data("blur-class");
					var rest_class	= $(".youth-programs").data("rest-class");
					...
					if( focus_class == rest_class ){
						
						focus_classes	= "program-hovering";
						blur_classes	= "program-blurred";
						rest_classes	= "program-resting";
						
					} else {
						
						focus_classes	= focus_class+" program-hovering";
						blur_classes	= blur_class+" program-blurred";
						rest_classes	= rest_class+" program-resting";
					...
					$(".youth-program").each( function( index ){
					    ...
					    $this.hover( function(){ //mouseover
							...
							//Make wider
							$this
								.removeClass(rest_classes)
								.addClass(focus_classes)
								.siblings()
									.removeClass(rest_classes)
									.addClass(blur_classes);
								
							console.log(rest_class);
							...
							//On mouseout
								...
								//Make normal width
								$this
									.removeClass(focus_classes)
									.addClass(rest_classes)
									.siblings()
										.removeClass(blur_classes)
										.addClass(rest_classes);
...
-->
	...
	<?php 
		...
		if(get_field('heroes')): while(has_sub_field('heroes')): ?>
			...
			<?php
				...
				if( $hero->link === '/welcome' ) { 
					$hero->kind = 'welcome';
				} elseif( $hero->link === '/legacy' ) {
					$hero->kind = 'legacy';
				} elseif( $hero->link === '/invite' ) {
					$hero->kind = 'invite';
				}
	...
	<?php if( $hero->kind !== 'text' && isset($hero->page_options->readmore) && $hero->page_options->readmore !== 'hidden' ): ?>
