...
function boxesVimeo($boxes) {
	...
	if( empty($boxes->source) ){
		$boxes->source = '/channel/gutschurch';
	}

	//$vid_url = parse_url($boxes->source);

	$json_url = 'https://vimeo.com/api/v2'.$boxes->source.'/videos.json';
	...
	foreach ($json as $video) {
		...
		$link = site_url().'/watch/?vid='.$video->id;
...
function boxVimeo($object) {
	...
	$json_url = 'https://vimeo.com/api/v2/video/'.$object->id.'.json';
...
function getLatestVideoID($album_id='2238693') {
    ...
    $output = $boxes->objects->video_0->id;
...
class Boxes {
	...
	static function handle_shortcode($atts) {
		...
		$has_watch = has_shortcode( $post->post_content, 'watch');
					...
					<?php if( $boxes->objects ): foreach($boxes->objects as $key => $box): ?>
						...
						<?php
							...
							if( empty($boxes->srcType) ){
								$box->srcType = 'src="'.plugin_dir_url(dirname(__FILE__)).'public/images/blank-hd.gif" data-lazy';
							} else {
							...
							if( $boxes->layout === 'masonry' && $box->index >= $boxes->amount ){
								...
								$box->srcType = 'src="'.get_template_directory_uri().'/assets/img/blank-hd.gif" data-src';
							}
						?>
